...
func main() {
	ce := ConfigEngine{data: make(map[interface{}]interface{})}
	err := ce.Load("E:\\Projects\\project-github\\little-go\\practices\\yml\\config.yml")
	if err != nil {
		log.Fatal(err)
	}
...
func (c *ConfigEngine) loadFromYaml(path string) error {
	yamlS, readErr := os.ReadFile(path)
	if readErr != nil {
		return readErr
	}
