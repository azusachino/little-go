...
func (nv NumberValidator) Validate(val interface{}) (bool, error) {
	num := val.(int)
	if num < nv.Min {
		return false, fmt.Errorf("should be greater than %v \n", nv.Min)
	}
	if num > nv.Max {
		return false, fmt.Errorf("should be smaller than %v \n", nv.Max)
	}
	return true, nil
}
...
func (pv PhoneValidator) Validate(val interface{}) (bool, error) {
	var phoneReg = regexp.MustCompile(`^\d{11}$`)
	if !phoneReg.MatchString(val.(string)) {
		return false, fmt.Errorf("is not a valid phone number")
	}
	return true, nil
}
