...
import (
	...
	"encoding/json"
	"errors"
	"github.com/azusachino/little-go/sample/go-redis-k8s/model"
...
func indexHandler(w http.ResponseWriter, _ *http.Request) {
	_, _ = w.Write([]byte("Welcome! Please hit the `/qod` API to get the quote of the day."))
}
...
func quoteOfTheDayHandler(client *redis.Client) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err == redis.Nil {
			...
			if err != nil {
				_, _ = w.Write([]byte("Sorry! We could not get the Quote of the Day. Please try again."))
				return
			}
...
func getQuoteFromAPI() (*model.QuoteResponse, error) {
	...
	if resp.StatusCode >= 200 && resp.StatusCode <= 299 {
		...
		_ = json.NewDecoder(resp.Body).Decode(quoteResp)
